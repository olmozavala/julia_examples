...
function plotResults(SOAR, Gauss, w, h)
    ...
    display(plot(a,b, layout=lay))
    # for i=1:Int(round(w/2)):min((w*h), 90)
    for i=1:w*20:w*h
    # for i=1:1:w*h
        lay = @layout[a b]
        cur_SOAR = reshape(SOAR[i,:], (w,h))
        cur_G = reshape(Gauss[i,:], (w,h))
        a = heatmap(transpose(cur_SOAR), title="SOAR $i", aspect_ratio=:equal)
        b = heatmap(transpose(cur_G), title="Gauss $i", aspect_ratio=:equal)
        display(plot(a,b, layout=lay))
        # println("Plotting $i")
    end
end
